...
class DownSampler(object):
	...
	def sample(self, stSampleSize, increRatio, qualityThres,
		...
		while sampleQuality < qualityThres and sampleSize < maxSampleNum:
			...
			thisSampleIdx = randint(maxSampleNum, size = sampleSize)
			thisSample = self._orgData[thisSampleIdx]
			sampleQuality = self._getQualityMetric(thisSample);
			qualityHist.append([sampleSize, sampleQuality]);
			newSampleSize = sampleSize * increRatio;
			newSampleSize = int(newSampleSize)
			if newSampleSize == sampleSize:
				newSampleSize += 1
			sampleSize = newSampleSize
			print(sampleSize)
		res = [thisSample, np.array(qualityHist)]
		if returnIdx:
			res.append(thisSampleIdx) 
		return res
