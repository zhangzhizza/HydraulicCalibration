...
cur_dir=`pwd`
bas_nam="`basename ${cur_dir}`/$1"
cur_dir_name=`basename ${cur_dir}`
arg_len=$#

python_args=`echo ${@:2:arg_len}`
arg_lis=`echo ${python_args} | sed -e "s|${cur_dir}|.|g"`
...
docker run \
  ...
  /usr/local/JModelica/bin/jm_python.sh /mnt/shared/${cur_dir_name}/${arg_lis}"
exit $?
