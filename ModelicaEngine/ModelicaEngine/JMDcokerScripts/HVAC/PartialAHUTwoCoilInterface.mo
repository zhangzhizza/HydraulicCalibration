partial model PartialAHUTwoCoilInterface
    ...
    Modelica.Media.Interfaces.PartialMedium "Medium 2 in the component"
      annotation (choices(
        ...
        choice(redeclare package Medium =
          ...
          X_a=0.40)
          "Propylene glycol water, 40% mass fraction")));
   replaceable package MediumWater2 = Buildings.Media.Water constrainedby
    Modelica.Media.Interfaces.PartialMedium "Medium 2 in the component"
      annotation (choices(
        choice(redeclare package Medium = Buildings.Media.Air "Moist air"),
        choice(redeclare package Medium = Buildings.Media.Water "Water"),
  ...
  parameter Modelica.SIunits.MassFlowRate water1_flow_nominal(min=0)
    ...
    annotation(Dialog(group = "Nominal condition"));
  parameter Modelica.SIunits.MassFlowRate water2_flow_nominal(min=0)
    "Nominal mass flow rate of water in coil 2 (kg/s)"
    annotation(Dialog(group = "Nominal condition"));
  ...
  parameter MediumWater1.MassFlowRate water1_flow_small(min=0) = 1E-4*abs(water1_flow_nominal)
    ...
    annotation(Dialog(tab = "Advanced"));
  parameter MediumWater2.MassFlowRate water2_flow_small(min=0) = 1E-4*abs(water2_flow_nominal)
    "Small mass flow rate for regularization of zero flow for water in coil 2"
    annotation(Dialog(tab = "Advanced"));
  // Diagnostics
  ...
  MediumWater2.MassFlowRate water2_flow=water_port_a2.m_flow
    "Mass flow rate from water_port_a2 to water_port_b1 (water2_flow > 0 is design flow direction)";
  Modelica.SIunits.PressureDifference water2_dp(displayUnit="Pa") = water_port_a2.p -
    water_port_b2.p "Pressure difference between water_port_a2 and water_port_b2";
  ...
  MediumWater2.ThermodynamicState sta_water_a2=MediumWater2.setState_phX(
      water_port_a2.p,
      noEvent(actualStream(water_port_a2.h_outflow)),
      noEvent(actualStream(water_port_a2.Xi_outflow))) if show_T
    "Medium properties in water_port_a2";
  MediumWater2.ThermodynamicState sta_water_b2=MediumWater2.setState_phX(
      water_port_b2.p,
      noEvent(actualStream(water_port_b2.h_outflow)),
      noEvent(actualStream(water_port_b2.Xi_outflow))) if show_T
    "Medium properties in water_port_b2";
  ...
  Modelica.Fluid.Interfaces.FluidPort_a water_port_a2(
    redeclare final package Medium = MediumWater2,
    m_flow(min=if allowFlowReversal2 then -Modelica.Constants.inf else 0),
    h_outflow(start=MediumWater2.h_default, nominal=MediumWater2.h_default))
    "Fluid connector a (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{138,84},{158,104}}),
        iconTransformation(extent={{138,84},{158,104}})));
  Modelica.Fluid.Interfaces.FluidPort_b water_port_b2(
    redeclare final package Medium = MediumWater2,
    m_flow(max=if allowFlowReversal2 then +Modelica.Constants.inf else 0),
    h_outflow(start=MediumWater2.h_default, nominal=MediumWater2.h_default))
    "Fluid connector b (positive design flow direction is from port_a to port_b)"
...
protected
  ...
  MediumWater2.ThermodynamicState state_water_a2_inflow=MediumWater2.setState_phX(
      water_port_a2.p,
      inStream(water_port_a2.h_outflow),
      inStream(water_port_a2.Xi_outflow))
    "state for medium inflowing through water_port_a2";
  MediumWater2.ThermodynamicState state_water_b2_inflow=MediumWater2.setState_phX(
      water_port_b2.p,
      inStream(water_port_b2.h_outflow),
      inStream(water_port_b2.Xi_outflow))
    "state for medium inflowing through water_port_b2";

  annotation (
  preferredView="info",
    Documentation(info="<html>
<p>
This component defines the interface for an air handling unit (AHU) model with two water coils.
</p>
</html>", revisions="<html>
<ul>
<li>
Mar 18, 2022, by Zhiang Zhang:<br/>
First creation.
</li>
</ul>
</html>"),
