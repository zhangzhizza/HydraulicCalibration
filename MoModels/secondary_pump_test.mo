...
model secondary_pump_test
  ...
  Buildings.Fluid.FixedResistances.Junction jun_19(
    ...
    dp_nominal={1000,-600,700}) annotation (Placement(transformation(
        extent={{8,8},{-8,-8}},
        rotation=90,
        origin={18,116})));
  ...
  Buildings.Fluid.FixedResistances.Pipe pipe_10(
    ...
    roughness=0.5)
        ...
        rotation=270,
        origin={18,66})));
  Buildings.Fluid.FixedResistances.Pipe bypass_pipe(
    redeclare package Medium = Buildings.Media.Water,
    energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    massDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    T_start=286.65,
...
equation
  connect(jun_19.port_2, pipe_10.port_a)
    annotation (Line(points={{18,108},{18,76}}, color={0,127,255}));
  connect(bypass_pipe.port_b, jun_19.port_3)
    annotation (Line(points={{-52,114},{10,116}}, color={0,127,255}));
  connect(secdy_ret_pipe.port_b, chw_ret.ports[1]) annotation (Line(points={{110,
          128},{156,128},{156,160},{168,160}}, color={0,127,255}));
  connect(secdy_ret_pipe.port_a, jun_19.port_1)
    annotation (Line(points={{90,128},{18,128},{18,124}}, color={0,127,255}));
  ...
  connect(pipe_9.port_b, jun_20.port_1) annotation (Line(points={{-144,70},{
          -144,90},{-146,90},{-146,110}}, color={0,127,255}));
  connect(jun_20.port_2, pipe_8.port_a)
    annotation (Line(points={{-146,126},{-164,126},{-164,148},{-144,148}},
                                                     color={0,127,255}));
  connect(jun_20.port_3, bypass_pipe.port_a) annotation (Line(points={{-138,118},
          {-88,118},{-88,114},{-76,114}}, color={0,127,255}));
